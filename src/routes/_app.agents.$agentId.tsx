...
import { ArrowDown, ArrowUp, ArrowLeft, DollarSign, Heart, LineChart as LineChartIcon, Package, Percent, ShieldPlus, Wallet, Users } from "lucide-react";
...
function AgentDashboardPage() {
  ...
  const canEditLicenses =
    !!user && (user.id === agentId || roles.includes("admin") || roles.includes("manager"));
  ...
  return (
    <div className="space-y-6">
      ...
      <StateLicenseMap agentId={agentId} editable={canEditLicenses} />
  ...
  return (
    <div className="surface-card p-5">
      <div className="flex items-center justify-between">
        <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Attach Rates</div>
        <Percent className="h-4 w-4 text-muted-foreground" />
      </div>
      <div className="mt-3 grid grid-cols-2 gap-4">
        <AttachRateStat label="Life" value={formatPct(lifeRate)} delta={lifeDelta} target={lifeTarget} />
        <AttachRateStat label="Add-on" value={formatPct(addonRate)} delta={addonDelta} target={addonTarget} />
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[11px]">
        ...
        {hasTarget && <span className="text-muted-foreground">Target: {formatPct(target!)}</span>}
      </div>
    </div>
  );
}
